...
class PubkeyEntry(Entry):
    ...
    def sign(self, private_key, certifying_id = None):
        ...
        if certifying_id:
            ...
            self.certifying_id = hexlify(cetifying_id)
    ...
    def verify_signature(self, pubkey=None):
        ...
        if pubkey:
            ...
            for key in filter(lambda key: len(key.modulus) != self.signature_len, pubkeys):
                print_warning(f"Key at {hex(key.get_address())} is supposed to sign key at: {hex(self.get_address())}, but signature/key sized don't match.")

            pubkeys = list(filter(lambda key: len(key.modulus) == self.signature_len, pubkeys))

            if not pubkeys:
                print_warning(f"Couldn't find sigining key for key at: {hex(self.get_address())}.")
                return
